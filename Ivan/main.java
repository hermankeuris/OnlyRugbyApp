...
public class main 
{
    ...
    public class Player 
    {
		...
		//A variable to store players clean breaks
		private int cleanBreaks;
        
        //Constructor with parameters
        Player(String pn, String pt, int jn, boolean field)
        {
            playerName = pn;
            playerTeam = pt;
            jerseyNum = jn;
		...
		//A function which changes the player's name
        public void setPlayerTeam(String pt)
        {
            playerTeam = pt;
        }
	...
	private int ownScrumsWon;
	private int ownScrumsLost;
		
	private int scrumsWon;
	private int scrumsLost;
	...
	public void incOwnScrumsWon()
	{
		ownScrumsWon++;
		scrumsWon++;
	}
		
	public void incOwnScrumsLost()
	{
		ownScrumsLost++;
		scrumsLost++;
	}
		
	public void incScrumsWon()
	{
		scrumsWon++;
	}
		
	public void incScrumsLost()
	{
		scrumsLost++;
	}
    }
    ...
    public void scrum (String owningTeam, String winningTeam)
    {
        //The team who won the scrum
        Team winner = null;
        Team loser = null;
        ...
        if (teams.get(0).getTeamName() == winningTeam)
        {
            winner = teams.get(0);
            loser = teams.get(1);
        }
        else if (teams.get(1).getTeamName() == winningTeam)
        {
            winner = teams.get(1);
            loser = teams.get(0);
        }
        ...
        if(winningTeam == owningTeam)
        {
            winner.incOwnScrumsWon();
            loser.incScrumsLost();
        }
        else
        {
            winner.incScrumsWon();
            loser.incOwnScrumsLost();
        }
		
        //Log scrum
    }
	
    public void clean_break(String playerName)
    {
        Player player = null;
		
        for(int i = 0; i <= 1; i++)//Find players team
        {
            for(int k = 0; k <= 14; k++)
            {
                if(teams.get(i).onField.get(k).getPlayerName() == playerName)
                {
                    player = teams.get(i).onField.get(k);
                }
            }
        }
        
        player.incCleanBreaks();
    }
    ...
    public void loadInfo ()
    {
    	...
    	for (int k = 0; k <= 1; ++k) //for the two (2) different teams
    	{
    		...
    		{
    			teams.get(k).onField.add(new Player("P" + i + "(T" + (k+1) + ")", teams.get(k).teamName, i, false)); //False, assuming the player is NOT a reserve player
    		}
    		
    		for (int i = 16; i <= 30; ++i) //for the fifteen (15) reserve players
    		{
    			teams.get(k).onField.add(new Player("P" + i + "(T" + (k+1) + ")", teams.get(k).teamName, i, true)); //False, assuming the player IS a reserve player
    		}
    	}
